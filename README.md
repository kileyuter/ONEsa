...
- 支持逐元素解析常见飞书富文本结构，包括标准 `post.zh_cn.content` 包装，优先保留可读文字、内联样式、列表、图片和表格。
...
v0.4.3 补齐了飞书标准 `post.zh_cn.content` 富文本包装解析，并在 `img_v3` 降级内容出现时增加指定消息接口二次拉取与 Debug 诊断；v0.4.2 改进了飞书富文本逐元素解析和降级策略：有可读文字时不再整条消息降级为“富内容”；v0.4.1 增加了收到消息后的图片资源即时预取；v0.4 增强了飞书消息内图片资源下载与本地缓存；v0.3 增强了飞书回复引用上下文、Markdown 简单表格和可抽取文本的富文本表格。调试日志、过程文档、原型文件、测试目录和构建产物不纳入版本。
