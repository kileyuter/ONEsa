...
enum UserFacingCopy {
    enum Connection {
        ...
        static let notConfiguredDetail = "请先在设置中填写飞书应用、目标 chat_id 和 OpenClaw sender。"
        static let notLoggedInDetail = "配置已保存，请完成飞书授权。"
        static let connectedDetail = "已准备好连接 OpenClaw，并将只读取匹配 sender 的回复。"
        ...
        static func chatStatus(for snapshot: FeishuConnectionSnapshot, hasMessages: Bool) -> String {
            ...
            if snapshot.hasValidUserAccessToken {
                return hasMessages ? "已恢复最近本地历史，可继续发送消息。" : "已连接，可向 OpenClaw 发送第一条消息。"
            }
    ...
    enum Configuration {
        ...
        static let missingSenderFilter = "请填写 OpenClaw sender id 和 sender type，避免误判其他消息。"
        ...
        static func summary(for snapshot: FeishuConnectionSnapshot) -> String {
            ...
            if !snapshot.configuration.hasOpenClawSenderFilter {
    ...
    enum Authorization {
        ...
        static let chatReady = "飞书授权已连接，可发送消息并等待 OpenClaw 回复。"
    ...
    enum Chat {
        ...
        static let connectedReady = "已连接，可向 OpenClaw 发送消息。"
        ...
        static let waitingReply = "消息已发送，正在持续轮询目标会话并等待 OpenClaw 回复；如果任务较长，后台会继续监听新消息。"
        static let replyReceivedPrefix = "已读取 OpenClaw 回复"
        static let timeoutSystemMessage = "当前任务等待超时：在限定时间内未读到 OpenClaw 回复，但后台仍会继续监听该会话中的新消息。"
        static let timeoutStatusMessage = "当前任务等待超时，后台仍会继续监听 OpenClaw 新消息。"
        static let waitStopped = "已停止当前前台等待；后台仍会继续监听 OpenClaw 新消息。"
        ...
        static let backgroundListening = "后台正在监听 OpenClaw 新消息。"
        ...
        static func backgroundReceived(count: Int, hasOfflineBadge: Bool) -> String {
            let prefix = count == 1
                ? "已在后台收到 1 条新的 OpenClaw 消息。"
                : "已在后台收到 \(count) 条新的 OpenClaw 消息。"
    ...
    enum Recovery {
        ...
        static let agentOfflineTitle = "OpenClaw 当前离线"
        ...
        static func configurationRequired(missingTitles: [String]) -> String {
            guard !missingTitles.isEmpty else {
                return "请先补齐飞书应用配置、目标会话和 OpenClaw sender 信息。"
            }
        ...
        static func agentOffline(_ message: String) -> String {
            "当前无法确认 OpenClaw 在线，已阻止继续发送。\(message)"
        }
